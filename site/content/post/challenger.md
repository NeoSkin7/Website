...
It's 1986, and NASA have a problem with their Challenger Space Shuttle[^1] - you see, it was cold when they planned to launch and they had an issue with the O-rings on the shuttle. Hold on, I'll show you!

The operating principles of a Space Shuttle are relatively simple; the main Orbiter vehicle is launched by two sets of rocket boosters[^2], powered by liquid hydrogen and oxygen stored in the famous orange external tank. Two minutes into launch[^3], the boosters fall away, and the Orbiter is powered by the three rocket engines.

By burning liquid hydrogen in thrusters in a large supply of oxygen, the Space Shuttle achieves the thrust needed for lift off. Now, the O-rings we're interested in sealed the fuel pipe joints on the shuttle. The hydrogen was burned here in the Solid Rocket Boosters, with the vital pipe joints located here - sealed with the O-rings[^4].

Now, NASA knew that their O-ring seals became less effective at lower temperatures, with a reduction in elasticity of 5 times between 75 and 30 degrees Fahrenheit - meaning that the O-rings are 5 times worse at returning to their original shape. Moreover, the air temperature at launch was 36 degrees Fahrenheit (roughly 2 degrees Celsius) - which was 15 degrees colder than any previous launch [^5]. Those were extremely worrying conditions, so they commissioned a graph[^6].

## The Fatal Graph

![The First Graph](/img/post/challenger/oring-incidents.png)

This is the graph NASA managers were presented with (Figure 6, above)[^7]. Can you find a correlation between the number of incidents and the joint temperature? NASA managers and engineers couldn't, and there is not a clear relationship here. So, they proceeded with the launch.
...
Let’s re-examine the data, previously we looked at this graph (Figure 6, above) - the same one NASA managers had been presented with. But, this isn’t the full story. In fact, if we include data from successful launches, we get this graph (Figure 7, below) - notice anything?
...
It has been just over 40 years since the Challenger disaster, and NASA now has more effective systems and protocols in place which aim to prevent future catastrophes[^8].

I’ve left more detailed information over on my website for those of you who are interested.
...
- Figures 6 and 7 reproduced from the _Report to the Presidential Commission_ [modified]
